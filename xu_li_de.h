...
int check_lop(DS_LOP ds_l,string a)
{
	for (int i = 0; i < ds_l.sl; i++)
	{
		if (ds_l.ds[i]->ma_lop == a)
		{
			if(ds_l.ds[i]->danh_sach_sv.p_head == NULL)return i;
		}
	}
	return -1;
}
...
void nhap_sinh_vien(DS_LOP &ds_l,DS_SINH_VIEN &ds_sv)
{
	...
	else
	{
		...
		tao_ma_hd:
			...
			them_1_sinh_vien(tam->danh_sach_sv.p_head, p);
			tam->danh_sach_sv.sl++;
			ds_sv.sl++;
			gotoxy(70, 30);
			cout << "                             ";
...
int check_ma_sv(DS_LOP ds_l, string ma)
{
	for (int i = 0; i < ds_l.sl; i++)
	{
		if (ds_l.ds[i]->danh_sach_sv.p_head == NULL)
		{
			return -1;
		}
...
void xoa_sv(DS_LOP &ds_l) // HAM XOA SV
{
	string ma_sv;
	...
	{
		SINH_VIEN* g = new SINH_VIEN;// node g là node trỏ đến node nằm trước node cần xoá
		for (int i = 0; i < ds_l.sl; i++)
		{
			for (SINH_VIEN *k = ds_l.ds[i]->danh_sach_sv.p_head; k != NULL; k = k->p_next)
			{
...
void in_ds_sv(DS_SINH_VIEN ds_sv, DS_LOP ds_l)
{
	gotoxy(60, 8);
	...
	for (int i = 0; i < ds_l.sl; i++)
	{
		for (SINH_VIEN *k = ds_l.ds[i]->danh_sach_sv.p_head; k != NULL; k = k->p_next)
		{
			gotoxy(45, 12 + 1 * j); cout << "| " << ds_l.ds[i]->ma_lop;
			gotoxy(60, 12 + 1 * j); cout << "| " << k->ma_sv;
			gotoxy(75, 12 + 1 * j); cout << "| " << k->ho << " " << k->ten;
			gotoxy(100, 12 + 1 * j); cout << "| " << k->phai;
...
int dem_sl_cau(string a, cau_hoi_thi *ds[], int &nds)
{
	int sl_cau = 0;
	for (int i = 0; i < nds; i++)
	{
...
check_mon:
	...
	//{
		...
		hour:
			...
			bo_de(ds, nds, a, so_cau, hour, minute, second);
		}
}
